...
@celery.task
def send_email_task(subject: str, message: str, details: dict):
    """Send an email asynchronously using Celery."""
    try:
        logger.info("send_email_task ---------------------------")
        email = EmailService()
        details = json.loads(details) if details else {}
        email.send_email(subject=subject, message=message, details=details)
        return f"Email {subject}  was sent successfully"
